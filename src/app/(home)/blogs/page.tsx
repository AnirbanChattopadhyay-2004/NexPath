...
export default async function (){
  ...
  return (
    <div className="flex flex-col items-center pt-4 text-white">
      <Link href={"/createBlog"} className="p-3 bg-white rounded-md text-black mb-3 font-semibold">Create Blog</Link>
      <div className="grid grid-cols-1">
        {blogs.map((blog: blog , ind : number) => <Blog blog={blog} key={ind}/>)}
      </div>
    </div>
  );
  ...
  return (
      <div className="p-4 m-2 mx-auto w-4/5 min-w-[400px] max-w-[400px] bg-gray-800 text-white rounded-lg shadow-md border border-gray-600">
          ...
          <div className="flex justify-between items-center">
            ...
            <Link href={"/blogs/" + blog.id} className="p-2 bg-white rounded-md text-black font-semibold">Read</Link>
          </div>
      </div>
  );
}