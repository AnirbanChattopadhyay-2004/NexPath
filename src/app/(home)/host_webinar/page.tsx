...
export default function DatePickerDemo() {
  ...
  return (
    ...
    <form className="w-2/4" action={async()=>{
        // setLoading(true);
        const[hr , min] = time.split(":").map(Number);
        date?.setHours(hr);
        date?.setMinutes(min);
        const res = await addWebinar(date as Date, title , description , userEmail as string);
        // setLoading(false);
        if(res == "error"){
...
function WebinarButton(){
  ...
  return(
    <button type="submit" className="p-2 bg-gray-700 rounded-md w-full hover:bg-gray-500">Create{pending ? "..." : ""}</button>
  )
}