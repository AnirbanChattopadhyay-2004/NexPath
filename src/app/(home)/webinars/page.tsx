...
export default async function Page(){
    ...
    const webinars = await getWebinar(user?.emailAddresses[0].emailAddress as string);
...
async function WebinarCard({ webinar }: { webinar: webinar }) {
    ...
    return (
        <div className="p-4 m-2 mx-auto w-4/5 min-w-[400px] max-w-[400px] bg-gray-800 text-white rounded-lg shadow-md border border-gray-600">
            ...
            <p className="text-gray-300 mb-2 line-clamp-2">{webinar.description}</p>
            <div className="text-gray-400 mb-3">{new Date(webinar.date).toLocaleString().toUpperCase()}</div>
            <div className="flex justify-between items-center">
                ...
                <Link href={"/room/" + webinar.id} className="p-2 bg-white rounded-md text-black font-semibold">Join</Link>
            </div>
        </div>
    );
}